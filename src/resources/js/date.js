(function ($) {
    ...
    var calendarSwitch = (function () {
        ...
        var secondMonthDays = null;
        var firstMonthDays = null;
        var thirdMonthDays = null;
        ...
        calendarSwitch.prototype = {
            ...
            init: function () {
                ...
                function getDate(strDate, callback) {
                    var st = strDate;
                    var a = st.split(" ");
                    var b = a[0].split("-");
                    var date = new Date(b[0], b[1], b[2]);
                    callback && callback(b[0], b[1], b[2]);
                    return date;
                }
                ...
                getDate(me.startDate, function (year, month, day) {
                    startYear = year;
                    startMonth = month;
                })

                getDate(me.endDate, function (year, month, day) {
                    endYear = year;
                    endMonth = month;
                });
                ...
                if (parseInt(endYear) > parseInt(startYear)) {
                    //cha = (parseInt(startYear) - parseInt(endYear)) * 12;
                    if (startMonth.substr(0, 1) == '0') {
                        startMonth = parseInt(startMonth.substr(1, 2));
                        if (endMonth.substr(0, 1) == '0') {
                            endMonth = parseInt(endMonth.substr(1, 2));
                        } else {
                            endMonth = parseInt(endMonth);
                        }
                        cha = (12 - startMonth) + endMonth;
                    } else {
                    ...
                    cha = endMonth - startMonth;
                }


                var html = "<div class='headerWrapper'><div class='headerTip'>请选择您考试时间</div><div class='comfire'>完成</div></div><table class='dateZone'><tr><td class='colo'>日</td><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td class='colo'>六</td></tr></table>" + "<div class='tbody'></div>"
                ...
                for (var q = 0; q < cha + 1; q++) {
                    var select = q;
                    $(me.sections).find(".tbody").append("<p class='ny1'></p><table class='dateTable'></table>")
                    //var currentDate = getDate(me.startDate, function () {});
                    var currentDate = new Date(me.startDate);
                    ...
                    if (q == 0) {
                        firstMonthDays = DaysInMonth[currentMonth];
                    }
                    if (q == 1) {
                        secondMonthDays = DaysInMonth[currentMonth];
                    }
                    if (q == 2) {
                        thirdMonthDays = DaysInMonth[currentMonth];
                    }
            ...
            _initselected: function () {
                ...
                tds.each(function (index, element) {
                    if ($(this).text() == strDays) {
                        var r = index;
                        $(this).append('</br><p style="display: none" class="rz">开始</p>');
                        if ($(this).next().text() != "") {
                            //$(this).next().append('</br><p style="display: none" class="rz">结束</p>');
                        } else {
                            $(".dateTable").eq(1).find("td").each(function (index, el) {
                                if ($(this).text() != "") {
                                    $(this).append('</br><p style="display: none" class="rz">结束</p>');
                                    return false;
                                }
                            });
                        }
                ...
                //其他两个月
                var allDay = firstMonthDays + secondMonthDays + thirdMonthDays;
                var theDay = firstMonthDays + secondMonthDays + strDays;
                for (var j = theDay; j < allDay; j++) {
                    $(arry[j]).css('color', '#ccc');
                }
                ...
                if (me.daysnumber) {
                    ...
                    for (var i = strDays - 1; i < theDay; i++) {
                        arry1.push(arry[i])
                    }
                }
            ...
            _selectDate: function (arry1) {
                ...
                $(arry1).on('click', function (index) {
                    ...
                    if (flag == 0) {
                        ...
                        $(this).append('<p style="display: none" class="rz">开始</p>')
                        ...
                        $(me.sections).find('.rz').each(function (index, element) {
                            if ($(this).text() == '开始') {
                                $(this).parent('td').append('<span class="hover ruzhu_hover">请选择结束日期</span>');
                        ...
                        if (first < second) {
                            $(this).append('<p style="display: none" class="rz">结束</p>')
                            first = first + 1;
